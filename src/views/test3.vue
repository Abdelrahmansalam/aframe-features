<template>
  ...
  AFRAME.registerComponent("gestures", {
    ...
    createMark: function (position) {
      ...
      label.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="red" class="bi bi-geo-alt-fill" viewBox="0 0 16 16"><path d="M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10m0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6"/></svg>';
    ...
    // Function to check if the object is in the camera's view frustum
    // Function to check if the target is in the camera's view frustum
    is_in_camera_frustrum: function (target, rig, camera) {
      const original_quaternion = new THREE.Quaternion();
      const e1 = new THREE.Euler();
      const e2 = new THREE.Euler();
      const result = new THREE.Vector2();
  
      const toDegree = (d) => THREE.MathUtils.radToDeg(d).toFixed(2);
  
      original_quaternion.copy(rig.quaternion);
  
      rig.updateMatrixWorld();
  
      e1.copy(rig.rotation);
  
      rig.lookAt(target);
      rig.rotateY(THREE.MathUtils.degToRad(180));
  
      rig.updateMatrixWorld();
  
      e2.copy(rig.rotation);
  
      rig.quaternion.copy(original_quaternion);
  
      rig.updateMatrixWorld();
  
      result.set(toDegree(e1.x - e2.x), toDegree(e1.y - e2.y));
  
      const in_x = result.x < camera.fov / 2;
      const in_y = result.y < (camera.fov * camera.aspect) / 2;
    ...
    // Add the shaft if it doesn't exist
    if (!this.shaft) {
      ...
      this.shaft.position.set(0, 0, -1.5);  // Position it 1.5 units in front of the camera
  
      camera.add(this.shaft);
    }
    ...
    this.shaft.localToWorld(shaftStart);
    this.shaft.localToWorld(shaftEnd);
    ...
    for (const mark of this.markings) {
      ...
      this.shaft.lookAt(mark.position);
